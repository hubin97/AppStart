...
extension Reactive where Base: BLEManager {
    ...
    // MARK: - 中心管理者状态
    /// 中心管理者状态更新
    public func managerStateUpdate() -> Observable<CBManagerState> {
    ...
    // MARK: - 扫描
    /// 扫描状态更新
    public func scanStateUpdate() -> Observable<BLEScanState> {
        return Observable.create { observer in
    ...
    // MARK: - 发现外设/解析广播包
    /// 常规发现外设
    public func peripheralDiscovered() -> Observable<(CBPeripheral, BLEPeripheraData)> {
        return Observable.create { observer in
    ...
    /// 仅在指定解析器时, 发现外设回调
    public func peripheralDiscoveredWithParser<T>() -> Observable<(CBPeripheral, BLEPeripheraData, T?)> {
        return Observable.create { observer in
    ...
    // MARK: - 连接
    /// 连接状态更新
    public func connectStateUpdate() -> Observable<(BLEConnectionState, CBPeripheral)> {
        return Observable.create { observer in
    ...
    public func reconnectPhaseUpdate() -> Observable<(CBPeripheral, BLEReconnectState)> {
        return Observable.create { observer in
    ...
    // MARK: 数据更新
    public func dataReceived() -> Observable<BLECharValueUpdateResult> {
        return Observable.create { observer in
    ...
    // MARK: 写指令
    /// 写入指令超时
    public func writeTimeout() -> Observable<BLEWriteData> {
        return Observable.create { observer in
